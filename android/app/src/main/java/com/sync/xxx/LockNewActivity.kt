...
:root{
  ...
  --card:#0a1418;
  --blue: #4f8cf7;
  --blue-glow: rgba(79, 140, 247, 0.35);
  --blue-dim: rgba(79, 140, 247, 0.08);
  --blue-brd: rgba(79, 140, 247, 0.2);
  --red: #f74f6b;
  --red-glow: rgba(247, 79, 107, 0.3);
  --text:#e0f0ff;
  --text2:#5a8acc;
  --text3:#1a2a4a;
}
...
body::after{
  ...
  background:repeating-linear-gradient(
    ...
    transparent,
    transparent 2px,
    rgba(79,140,247,.015) 2px,
    rgba(79,140,247,.015) 4px
  );
  pointer-events:none;
  z-index:1;
...
body::before{
  content:'';position:fixed;inset:0;
  background:
    radial-gradient(ellipse 80% 50% at 50% -5%,rgba(79,140,247,.14) 0%,transparent 60%),
    radial-gradient(ellipse 50% 40% at 10% 90%,rgba(247,79,107,.1) 0%,transparent 55%),
    radial-gradient(ellipse 50% 35% at 90% 85%,rgba(79,140,247,.08) 0%,transparent 55%);
  pointer-events:none;
  z-index:0;
}
...
@keyframes pulseBlue{
  0%,100%{filter:drop-shadow(0 0 8px #4f8cf7) drop-shadow(0 0 24px #4f8cf7)}
  50%{filter:drop-shadow(0 0 20px #4f8cf7) drop-shadow(0 0 50px #4f8cf7)}
}
...
.intro-bar-wrap{
  ...
  background:rgba(247,79,107,.15);
  border-radius:2px;
  overflow:hidden;
}
.intro-bar{
  height:100%;width:0%;
  background:linear-gradient(90deg,#4f8cf7,#f74f6b,#4f8cf7);
  border-radius:2px;
  animation:barFill 2.5s ease .5s forwards;
  box-shadow:0 0 10px #4f8cf7;
}
...
.virus-bg{
  ...
  background:radial-gradient(circle at 40% 35%,rgba(79,140,247,.12),rgba(20,30,80,.05));
  border:1px solid rgba(79,140,247,.22);
  display:flex;align-items:center;justify-content:center;
  position:relative;z-index:1;
  box-shadow:0 0 0 1px rgba(0,0,0,.5),0 0 40px rgba(79,140,247,.18),inset 0 1px 0 rgba(100,180,255,.08);
  animation:virusPulse 2.5s ease-in-out infinite;
}
@keyframes virusPulse{
  0%,100%{box-shadow:0 0 0 1px rgba(0,0,0,.5),0 0 30px rgba(79,140,247,.15),inset 0 1px 0 rgba(100,180,255,.08)}
  50%{box-shadow:0 0 0 1px rgba(0,0,0,.5),0 0 60px rgba(79,140,247,.35),0 0 90px rgba(79,140,247,.1),inset 0 1px 0 rgba(100,180,255,.12)}
}
...
.lock-title{
  ...
  text-shadow:0 0 20px rgba(79,140,247,.3);
}
.lock-sub{
  font-family:'JetBrains Mono',monospace;font-size:9px;
  color:#1a3a5a;letter-spacing:2.5px;text-transform:uppercase;
...
.pin-dot{
  ...
  border:1.5px solid rgba(79,140,247,.2);
  background:transparent;
  transition:background .18s,border-color .18s,box-shadow .18s,transform .18s;
}
.pin-dot.filled{
  background:var(--blue);border-color:var(--blue);
  box-shadow:0 0 14px rgba(79,140,247,.8),0 0 28px rgba(79,140,247,.4);
  transform:scale(1.12);
}
.pin-dot.error{
  border-color:#f74f6b;background:rgba(247,79,107,.3);
  box-shadow:0 0 12px rgba(247,79,107,.6);
...
.pin-status{
  ...
  text-align:center;color:#1a3a5a;transition:color .2s;
}
.pin-status.err{color:#f74f6b}
.pin-status.ok{color:var(--blue);text-shadow:0 0 10px var(--blue)}
...
.key{
  ...
  border:1px solid rgba(79,140,247,.06);
  background:linear-gradient(160deg,rgba(79,140,247,.04),rgba(20,30,80,.03));
  color:var(--text);
  display:flex;flex-direction:column;align-items:center;justify-content:center;
  cursor:pointer;-webkit-tap-highlight-color:transparent;
  transition:transform .1s,background .1s,border-color .1s,box-shadow .1s;
  position:relative;overflow:hidden;user-select:none;
  box-shadow:0 2px 8px rgba(0,0,0,.5),inset 0 1px 0 rgba(79,140,247,.03);
}
.key::before{
  content:'';position:absolute;top:0;left:10%;right:10%;height:1px;
  background:linear-gradient(90deg,transparent,rgba(79,140,247,.07),transparent);
}
.key .num{font-size:22px;font-weight:700;line-height:1;letter-spacing:-.3px;color:#e0f0ff;}
.key .sub{font-family:'JetBrains Mono',monospace;font-size:7px;letter-spacing:2px;color:#1a3a5a;margin-top:3px}
...
.key:active{
  transform:scale(.93);
  background:linear-gradient(160deg,rgba(79,140,247,.14),rgba(20,60,120,.1));
  border-color:rgba(79,140,247,.3);
  box-shadow:0 0 20px rgba(79,140,247,.15),inset 0 1px 0 rgba(79,140,247,.1);
}

/* ── INTRO COUNTER HACK ── */
.intro-pct-wrap{
  font-family:'JetBrains Mono',monospace;
  font-size:10px;letter-spacing:2px;color:#4f8cf7;
...
.data-col{
  ...
  font-family:'JetBrains Mono',monospace;
  font-size:11px;color:rgba(79,140,247,.15);
  line-height:1.4;
...
<div id="introScreen">
  <div class="intro-skull">
    <svg width="72" height="72" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <ellipse cx="32" cy="26" rx="18" ry="17" fill="none" stroke="#4f8cf7" stroke-width="2"/>
      <ellipse cx="24" cy="24" rx="5.5" ry="6" fill="rgba(247,79,107,0.25)" stroke="#f74f6b" stroke-width="1.5"/>
      <ellipse cx="40" cy="24" rx="5.5" ry="6" fill="rgba(247,79,107,0.25)" stroke="#f74f6b" stroke-width="1.5"/>
      <path d="M30 32 L32 28 L34 32 Z" fill="rgba(247,79,107,0.4)" stroke="#f74f6b" stroke-width="1"/>
      <path d="M14 38 Q14 48 22 48 L22 52 L26 52 L26 48 L32 48 L32 52 L36 52 L36 48 L42 48 Q50 48 50 38" stroke="#4f8cf7" stroke-width="2" fill="none" stroke-linecap="round"/>
      <line x1="26" y1="48" x2="26" y2="42" stroke="#4f8cf7" stroke-width="1.5"/>
      <line x1="32" y1="48" x2="32" y2="42" stroke="#f74f6b" stroke-width="1.5"/>
      <line x1="38" y1="48" x2="38" y2="42" stroke="#4f8cf7" stroke-width="1.5"/>
      <path d="M14 38 Q32 44 50 38" stroke="#4f8cf7" stroke-width="1.5" fill="none"/>
    </svg>
  </div>

  <div class="intro-title">
    <span class="line line1">HII BRO</span>
    <span class="line line2">REXTERNAL SYSTEM</span>
...
<div class="crack-layer">
  <svg viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
    <!-- Main cracks -->
    <g stroke="#4f8cf7" stroke-linecap="round" fill="none">
...
<div id="mainContent">
  <div class="virus-wrap">
    ...
    <div class="virus-bg">
      ...
      <svg class="virus-svg" width="52" height="52" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
        <!-- Core circle -->
        <circle cx="50" cy="50" r="22" fill="rgba(79,140,247,.08)" stroke="#4f8cf7" stroke-width="1.8"/>
        <!-- Inner DNA-like strands -->
        <circle cx="50" cy="50" r="13" fill="rgba(79,140,247,.06)" stroke="#4f8cf7" stroke-width="1.2" stroke-dasharray="4 3"/>
        <!-- Center dot -->
        <circle cx="50" cy="50" r="4" fill="rgba(79,140,247,.5)" stroke="#4f8cf7" stroke-width="1"/>
        <!-- Spikes (12 spikes) -->
        <line x1="50" y1="22" x2="50" y2="10" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="50" cy="8" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <line x1="72" y1="33" x2="81" y2="24" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="83" cy="22" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <line x1="78" y1="50" x2="90" y2="50" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="92" cy="50" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <line x1="72" y1="67" x2="81" y2="76" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="83" cy="78" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <line x1="50" y1="78" x2="50" y2="90" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="50" cy="92" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <line x1="28" y1="67" x2="19" y2="76" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="17" cy="78" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <line x1="22" y1="50" x2="10" y2="50" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="8" cy="50" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <line x1="28" y1="33" x2="19" y2="24" stroke="#4f8cf7" stroke-width="1.8" stroke-linecap="round"/>
        <circle cx="17" cy="22" r="3.5" fill="rgba(79,140,247,.3)" stroke="#4f8cf7" stroke-width="1.2"/>

        <!-- Diagonal spikes (shorter) -->
        <line x1="61" y1="24" x2="66" y2="16" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="68" cy="13" r="2.5" fill="#4f8cf7" opacity=".5"/>
        <line x1="76" y1="39" x2="84" y2="34" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="86" cy="32" r="2.5" fill="#4f8cf7" opacity=".5"/>
        <line x1="76" y1="61" x2="84" y2="66" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="86" cy="68" r="2.5" fill="#4f8cf7" opacity=".5"/>
        <line x1="61" y1="76" x2="66" y2="84" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="68" cy="87" r="2.5" fill="#4f8cf7" opacity=".5"/>
        <line x1="39" y1="76" x2="34" y2="84" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="32" cy="87" r="2.5" fill="#4f8cf7" opacity=".5"/>
        <line x1="24" y1="61" x2="16" y2="66" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="14" cy="68" r="2.5" fill="#4f8cf7" opacity=".5"/>
        <line x1="24" y1="39" x2="16" y2="34" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="14" cy="32" r="2.5" fill="#4f8cf7" opacity=".5"/>
        <line x1="39" y1="24" x2="34" y2="16" stroke="#4f8cf7" stroke-width="1.2" stroke-linecap="round"/>
        <circle cx="32" cy="13" r="2.5" fill="#4f8cf7" opacity=".5"/>
      </svg>
    </div>
  </div>
  ...
  <div class="pin-status" id="pinStatus">Creator GupongOfficial</div>
